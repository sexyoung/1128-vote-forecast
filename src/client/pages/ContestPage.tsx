...
const sharePlatformLabels: Record<SharePlatform, string> = {
  facebook: 'Facebook',
  line: 'LINE',
  threads: 'Threads',
  twitter: 'Twitter',
...
export function ContestPage() {
  ...
  const contestTitle = resolved
    ...
    : '選區';
  const [leader, runnerUp] = detail.data?.tally.rows ?? [];
  const timestamped = /^\d{10,13}$/.test(searchParams.get('t') ?? '');
  const sharedLeader =
    timestamped && resolved?.contest.seatCount === 1 && leader?.count > (runnerUp?.count ?? 0)
      ? leader
      : null;
  useDocumentTitle(
    `${resolved?.contest.seatCount === 1 ? `${contestTitle}最多人預測的是${sharedLeader?.label ?? '.....'}` : `${contestTitle}預測`}｜九合一選舉預測`,
  );
  ...
  return (
    <PageShell>
      <main className="page contest-page">
        ...
        <section aria-label="分享這個選區" className="contest-share">
          <span>分享</span>
          {(['facebook', 'threads', 'twitter', 'line'] as const).map((platform) => (
            <button
              aria-label={`分享到${sharePlatformLabels[platform]}`}
              className={platform}
              key={platform}
              onClick={() => {
                window.open(
            ...
            >
              ...
              <span>{sharePlatformLabels[platform]}</span>
            </button>
          ))}
        </section>
